...
test("adversarial review command uses AskUserQuestion and background Bash while staying review-only", () => {
  ...
  assert.match(source, /\[--scope auto\|working-tree\|branch\] \[--effort <none\|minimal\|low\|medium\|high\|xhigh>\] \[focus \.\.\.\]/);
  assert.match(source, /run_in_background:\s*true/);
...
test("rescue command absorbs continue semantics", () => {
  ...
  assert.match(rescue, /runtime defaults them to `gpt-5\.6-terra` and `high`/i);
  ...
  assert.match(agent, /runtime defaults them to `gpt-5\.6-terra` and `high`/i);
  ...
  assert.match(runtimeSkill, /runtime defaults them to `gpt-5\.6-terra` and `high`/i);
  ...
  assert.match(readme, /plugin uses `gpt-5\.6-terra` with `high` reasoning effort/i);
...
test("plan review commands select an explicit model and effort from plan complexity", () => {
  const reviewPlan = read("commands/review-plan.md");
  const refinePlan = read("commands/refine-plan.md");

  for (const source of [reviewPlan, refinePlan]) {
    assert.match(source, /gpt-5\.6-luna.*medium/i);
    assert.match(source, /gpt-5\.6-terra.*high/i);
    assert.match(source, /gpt-5\.6-sol.*xhigh/i);
    assert.match(source, /--model <selected-model> --effort <selected-effort>/i);
  }
});

test("plan review commands pass relevant repository context to Codex", () => {
  const reviewPlan = read("commands/review-plan.md");
  const refinePlan = read("commands/refine-plan.md");
  const prompt = read("prompts/review-plan.md");

  for (const source of [reviewPlan, refinePlan]) {
    assert.match(source, /source, test, configuration, and documentation files/i);
    assert.match(source, /supporting_context/i);
    assert.match(source, /\{\{SUPPORTING_CONTEXT\}\}/);
  }
  assert.match(prompt, /<supporting_context>/i);
  assert.match(prompt, /\{\{SUPPORTING_CONTEXT\}\}/);
});
