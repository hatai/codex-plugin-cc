...
- Leave `--model` and `--effort` unset unless the user explicitly requests overrides; the runtime defaults them to `gpt-5.6-terra` and `high`. If they ask for `spark`, map it to `gpt-5.3-codex-spark`.
